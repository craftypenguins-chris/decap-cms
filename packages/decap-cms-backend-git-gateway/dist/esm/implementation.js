...
import ini from 'ini';
import { APIError, unsentRequest, basename, entriesByFiles, parsePointerFile, getLargeMediaPatternsFromGitAttributesFile, getPointerFileForMediaFileObj, getLargeMediaFilteredMediaFiles, AccessTokenError, PreviewState } from 'decap-cms-lib-util';
...
export default class GitGateway {
  ...
  async status() {
    const api = await fetch(this.gitGatewayStatusEndpoint).then(res => res.json()).then(res => {
    ...
    if (api) {
      auth = (await this.tokenPromise?.().then(token => !!token).catch(e => {
  ...
  requestFunction = req => this.tokenPromise().then(token => unsentRequest.withHeaders({
    Authorization: `Bearer ${token}`
  }, req)).then(unsentRequest.performRequest);
    ...
    return this.tokenPromise().then(async token => {
      if (!this.backendType) {
        const {
          ...
          roles
        } = await unsentRequest.fetchWithTimeout(`${this.gatewayUrl}/settings`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        }).then(async res => {
  ...
  _getLargeMediaClient() {
    const netlifyLargeMediaEnabledPromise = this.api.readFile('.lfsconfig').then(config => ini.decode(config)).then(({
      lfs: {
        url
      }
    ...
    const lfsPatternsPromise = this.api.readFile('.gitattributes').then(attributes => getLargeMediaPatternsFromGitAttributesFile(attributes)).then(patterns => ({
      err: null,
      patterns
    })).catch(err => {
  ...
  async mirrorPersistEntry(entry, options) {
    if (!this.mirror?.url) return;
    try {
  ...
  async mirrorPersistMedia(mediaFile, options) {
    if (!this.mirror?.url) return;
    try {
  ...
  async updateUnpublishedEntryStatus(collection, slug, newStatus) {
    ...
    if (newStatus === 'rejected') {
      try {
        ...
        const entryData = await this.backend.unpublishedEntry({
          collection,
          slug
        });
