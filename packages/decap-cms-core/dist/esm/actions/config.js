...
export async function detectMirrorProxy(localPreviewMirror) {
  if (!localPreviewMirror?.url) {
    return {};
  }
  const allowedHosts = ['localhost', '127.0.0.1', ...(localPreviewMirror.allowed_hosts || [])];
  ...
  try {
    ...
    if (typeof repo === 'string' && typeof type === 'string') {
      ...
      return {
        mirrorProxyUrl: localPreviewMirror.url
      };
    } else {
