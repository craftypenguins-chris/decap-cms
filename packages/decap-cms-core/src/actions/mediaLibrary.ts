...
const MEDIA_DEBUG_VERSION = 'client-2025-09-21-01';
...
export function insertMedia(mediaPath: string | string[], field: EntryField | undefined) {
  return (dispatch: ThunkDispatch<State, {}, AnyAction>, getState: () => State) => {
    ...
    try { console.log('[mediaLibrary] insertMedia called:', mediaPath); } catch (_) {}
    ...
    async function ensureInRepo(pathOrPaths: string | string[]) {
      ...
      async function copyOne(singlePath: string) {
        try {
          ...
          try { console.log('[mediaLibrary] insertMedia getMediaFile status:', res.status); } catch (_) {}
    ...
    (async () => {
      ...
      try { console.log('[mediaLibrary] insertMedia final public path:', pathToInsert); } catch (_) {}
...
export function loadMedia(
  ...
  return async (dispatch: ThunkDispatch<State, {}, AnyAction>, getState: () => State) => {
    try { console.log('[mediaLibrary] loadMedia opts:', opts); } catch (_) {}
    const state = getState();
    ...
    if (integration) {
      ...
      try {
        ...
        try { console.log('[mediaLibrary] integration files:', files?.length); } catch (_) {}
    ...
    function loadFunction() {
      ...
      try { console.log('[mediaLibrary] loadFunction source:', source, 'subpath:', subpath); } catch (_) {}
      if (source === 'repo') {
        return backend
          .getMedia()
          .then(files => { try { console.log('[mediaLibrary] repo files:', files?.length); } catch (_) {}; return dispatch(mediaLoaded(files)); })
          .catch((error: { status?: number }) => {
            console.error(error);
            if (error.status === 404) {
              try { console.log('[mediaLibrary] repo media 404 -> empty'); } catch (_) {}
      ...
      try {
        ...
        return fetch(mirrorUrl as string, {
          ...
          .then(r => { try { console.log('[mediaLibrary] mirror status:', r.status); } catch (_) {}; return r.json(); })
          .then(json => {
